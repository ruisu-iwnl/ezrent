        ...
        <form method="POST" action="{{ route('payments.store') }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="payment[amount]" type="number" step="0.01" class="w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-700 @error('payment.amount') border-red-500 @enderror" placeholder="0.00" value="{{ old('payment.amount') }}" required>
                ...
                <div>
                    ...
                    <select name="payment[method]" class="w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-700 @error('payment.method') border-red-500 @enderror" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="payment[paid_at]" type="datetime-local" class="w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-700 @error('payment.paid_at') border-red-500 @enderror" value="{{ old('payment.paid_at') }}" required>
                ...
                <div>
                    ...
                    <input name="payment[reference]" type="text" class="w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-700 @error('payment.reference') border-red-500 @enderror" placeholder="Transaction ID, Check #, etc." value="{{ old('payment.reference') }}">
                ...
                <div>
                    ...
                    <input name="payment[receipt]" type="file" accept="image/*" class="w-full text-sm @error('payment.receipt') border-red-500 @enderror">
                    <p class="text-xs text-gray-500 mt-1">Upload receipt image (JPG, PNG, etc.)</p>
                    @error('payment.receipt')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label class="text-sm block mb-1">Notes</label>
                    <textarea name="payment[notes]" rows="3" class="w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-700 @error('payment.notes') border-red-500 @enderror" placeholder="Additional payment details (optional)">{{ old('payment.notes') }}</textarea>
                    @error('payment.notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
